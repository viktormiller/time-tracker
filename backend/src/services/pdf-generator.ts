 ...
 */
function generateReportHTML(entries: TimeEntry[], dateRange: DateRange, totalHours: number): string {
  const entriesRows = entries.map(entry => {
    const entryDate = new Date(entry.date);
    const formattedDate = format(entryDate, 'dd.MM.yyyy HH:mm', { locale: de });
  ...
  const fromDate = format(new Date(dateRange.from), 'dd.MM.yyyy', { locale: de });
  const toDate = format(new Date(dateRange.to), 'dd.MM.yyyy', { locale: de });
  const generatedAt = format(new Date(), 'dd.MM.yyyy HH:mm:ss', { locale: de });
